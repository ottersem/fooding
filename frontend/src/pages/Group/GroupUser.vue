<template>
    <v-container class="pa-0">
        <v-row no-gutters class="filter-box">
          <v-col cols="12" class="justify-start | align-center">
            <v-icon color="#364153" size="18" class="mr-1">$cus-filter-line</v-icon>
            <span class="description-text">상태 필터</span>
          </v-col>
          <v-col>
             <v-btn
              variant="outlined"
              :class="['filter-btn', selectedFilter === 'applied' ? 'filter-btn-selected' : 'filter-btn-unselected']"
              @click="selectedFilter = 'applied'"
            >
              <div style="display: flex; flex-direction: column; align-items: center;">
                <div class="filter-number">{{ groupStat.applied }}</div>
                <div class="filter-label">신청</div>
              </div>
            </v-btn>
          </v-col>
          <v-col>
            <v-btn
              variant="outlined"
              :class="['filter-btn', selectedFilter === 'hosting' ? 'filter-btn-selected' : 'filter-btn-unselected']"
              @click="selectedFilter = 'hosting'"
            >
              <div style="display: flex; flex-direction: column; align-items: center;">
                <div class="filter-number">{{ groupStat.hosting }}</div>
                <div class="filter-label">모집중</div>
              </div>
            </v-btn>
          </v-col>
          <v-col>
            <v-btn
              variant="outlined"
              :class="['filter-btn', selectedFilter === 'ongoing' ? 'filter-btn-selected' : 'filter-btn-unselected']"
              @click="selectedFilter = 'ongoing'"
            >
              <div style="display: flex; flex-direction: column; align-items: center;">
                <div class="filter-number">{{ groupStat.ongoing }}</div>
                <div class="filter-label">진행중</div>
              </div>
            </v-btn>
          </v-col>
          <v-col>
            <v-btn
              variant="outlined"
              :class="['filter-btn', selectedFilter === 'completed' ? 'filter-btn-selected' : 'filter-btn-unselected']"
              @click="selectedFilter = 'completed'"
            >
              <div style="display: flex; flex-direction: column; align-items: center;">
                <div class="filter-number">{{ groupStat.completed }}</div>
                <div class="filter-label">완료</div>
              </div>
            </v-btn>
          </v-col>
        </v-row>
...
const selectedFilter = ref('applied');
...
.filter-btn-unselected .filter-label {
  color: #6A7282 !important;
  font-size: 12px !important;
  font-weight: 450 !important;
}
